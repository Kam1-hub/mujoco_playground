...
44. Fresh env1024 R3 100k reset-calm AngVelXY diagnostic
45. 10M-scale training
...
- Step 41 passed as a deterministic render diagnostic. The sweep covered the
  same 100k push-disable, phase-freeze, and feet-air-time-mask checkpoints with
  fixed `fwd0.5`, `fwd1.0`, and stand commands. It produced `9` matching
  `.json`/`.mp4` pairs under `./logs/sac_render_terminal_diag_100k/`; all
  cases terminated via fall at step `51-52`, with negative torso-up z, negative
  root height, high torso XY angular velocity, and no contact/NaN reason.
- Step 42 passed runtime/checkpoint/eval/render gates with a meaningful but
  incomplete stability signal: fresh env1024 R3 100k reset-calm
  `--env_reset_joint_noise_scale 0.0 --env_reset_root_qvel_scale 0.0`
  returned `TRAIN_OK`, checkpoint readiness PASS, and fixed-command eval/render
  diagnostics with no NaN flags. Deterministic terminal renders delayed first
  fall from the previous `51-52` steps to step `68`, and train
  `done_fraction` improved to `0.00391`. The gate still failed because
...
- Step 44 passed runtime/checkpoint/eval/render gates but failed the stability
  gate: fresh env1024 R3 100k reset-calm plus
  `--env_reward_ang_vel_xy_scale -0.5` returned `TRAIN_OK`, checkpoint
  readiness PASS, and fixed-command eval/render diagnostics with no NaN flags.
  Fall timing did not materially improve versus reset-calm: deterministic eval
  first done stayed `68.5 / 68.75 / 68.5`, deterministic render first done
  stayed `68 / 69 / 68`, all fixed commands still fell, and deterministic
  rewards worsened materially versus reset-calm.
- Step 45 remains `NOT VALIDATED` and requires reward/prior targeted audit,
...
- Latest recorded validation state before this report update: bounded
  ...
  The latest termination/contact and terminal render sweeps now indicate those
  100k failures are early torso/base fall failures, not illegal contact or NaN.
...
## 29. Fresh Env1024 R3 100k Reset-Calm AngVelXY Diagnostic

Status: `TRAIN_OK`.

- Variant: reset-calm plus `--env_reward_ang_vel_xy_scale -0.5`.
- No action-rate override was included.
- Checkpoint:
