...
| 500k and 1M runs not validated | validation scope | OPEN | GPU 10k smoke, 4x200 deterministic eval smoke, 50k sanity/eval, 100k sanity/eval, and 250k sanity/eval passed; 500k and 1M have not been executed. | Next step should be a separate 500k sanity plan and user confirmation; do not jump straight to 1M. |
| 250k metrics need trend monitoring | training dynamics | OPEN_NON_BLOCKING | At 250k, alpha dropped to about `0.0187`; Q/target Q rose to about `5.2` while critic loss stayed low; bounded eval reward did not improve versus 100k. | Treat as a watch item, not a failure. In 500k planning, watch alpha collapse, Q drift, critic loss, eval NaN flags, and reward trend. |
...
- 500k, 1M, full eval benchmark, and PPO comparison are still `NOT VALIDATED`.
