...
| 128 envs may be too conservative for G1 SAC throughput | training capacity | OPEN | The current loop inserts `num_envs` transitions per actor step and runs only `grad_updates_per_step` updates. Keeping `batch_size=256` and `grad_updates_per_step=2` while moving from 128 to 1024 envs would drop sampled UTD by 8x. | Run 512/1024/2048 capacity benchmarks with coordinated `grad_updates_per_step` values 8/16/32 before longer training. |
